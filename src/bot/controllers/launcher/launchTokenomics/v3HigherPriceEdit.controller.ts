...
export const enterScene = async (ctx: any) => {
    ...
    const { message_id } = await ctx.reply(`<b>Enter the amount of Higher Price  </b>\n` + `This is the the percent of current price. \n` + `<i>(example: 300)</i>`, {
        parse_mode: 'HTML',
        reply_markup: {
            force_reply: true,
...
export const textHandler = async (ctx: any) => {
    ...
    try {
        ...
        id.length > 1 ? await Launches.findOneAndUpdate({ _id: id }, { higherPrice: _value }, { new: true }) : await Launches.findOneAndUpdate({ userId: ctx.chat.id, enabled: false }, { higherPrice: _value }, { new: true, upsert: true })
